...
def deploy_contract(token_name, token_symbol, initial_supply):
    ...
    construct_txn = contract.constructor(token_name, token_symbol, initial_supply).build_transaction(transaction)
    
    # Sign the transaction
    signed_tx = web3.eth.account.sign_transaction(construct_txn, private_key=PRIVATE_KEY)
    
    # Send the transaction - THIS IS WHERE THE FIX IS
    try:
